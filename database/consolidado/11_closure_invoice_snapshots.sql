...
create or replace function public.get_booking_closure_invoices(
  account_id_value uuid default null,
  session_token_value text default null,
...
begin
  ...
  return coalesce((
    select jsonb_agg(to_jsonb(invoice_rows) order by invoice_rows.invoiced_at)
    from (
      select invoices.*
      from public.booking_closure_invoices invoices
      where invoices.company_id = target_company_id
        and (
          public.is_admin()
          or account_record.role = 'admin'::public.app_role
          or exists (
            select 1
            from public.booking_closure_items closure_items
            where closure_items.closure_id = invoices.closure_id
              and closure_items.employee_id = account_record.employee_id
          )
        )
    ) invoice_rows
  ), '[]'::jsonb);
...
grant execute on function public.get_booking_closure_invoices(uuid, text, text) to anon, authenticated;

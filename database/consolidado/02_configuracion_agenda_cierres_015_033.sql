...
begin
  if TG_OP = 'INSERT' and NEW.status in ('reserved', 'confirmed', 'pending_assignment') then
    ...
    if NEW.employee_id is not null then
      ...
      employee_message := public.format_booking_notification_message(NEW, 'TenÃ©s un nuevo turno asignado.');
      perform public.send_resend_email(employee_email, 'TenÃ©s un nuevo turno asignado', employee_message, 'Quiero Turno App - No responder', NEW.user_email);
    end if;
  end if;
  ...
  if TG_OP = 'UPDATE'
    ...
    employee_message := public.format_booking_notification_message(NEW, 'Se cancelÃ³ este turno.');
    perform public.send_resend_email(employee_email, 'Se cancelÃ³ un turno', employee_message, 'Quiero Turno App - No responder', NEW.user_email);
  end if;

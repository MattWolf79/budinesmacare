-- ============================================================================
-- Source: database/consolidado/38_admin_panel_30d_window.sql
-- ============================================================================

-- Reduce payload size for admin panel data.
-- Bookings and closure datasets are limited to a rolling 30-day window:
-- 15 days back + 15 days forward from current business date (AR timezone).
...
begin
  ...
  -- 15/15 day window for admin UI payload
  window_start := (current_business_date - interval '15 day')::timestamp;
  window_end := (current_business_date + interval '16 day')::timestamp;
    ...
    'bookings', coalesce((
      ...
      from (
        ...
        where bookings.company_id = target_company_id
          ...
          and bookings.start_at < window_end
